...
K = eye(num_labels)(y, :);  % important 5000 x 10

% J = (1/m) * sum(sum((-K .* log(h)) - ((1-K) .* log(1-h))));
cost = sum((-K .* log(h)) - ((1-K) .* log(1-h)),2); %각 항들 묶기
J = (1/ m) * sum(cost);
...
D3 = (a3 - K); % 5000 x 10
Theta2_grad += (D3' * a2) ./ m; % 10 x 5000 * 5000 x 26 = 10 x 26
Theta2_grad(:, 2:end) += Theta2(:, 2:end) .* (lambda / m);

D2 =  (D3 * Theta2)(:, 2:end) .* sigmoidGradient(z2); % 5000 x 25
Theta1_grad += (D2' * a1) ./ m; % partial derivatives of each parameter in Theta1 => 25 x 401
